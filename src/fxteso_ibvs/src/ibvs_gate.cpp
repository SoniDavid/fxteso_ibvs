// Exits 0 once closed-loop servoing is actually possible: quad placed, target placed,
// markers decoded. sim.launch.py starts pos_ctrl / att_ctrl on that exit.
...
int main(int argc, char **argv)
{
	...
	while (rclcpp::ok())
	{
		...
		if (have_quad && have_tgt && lock_run >= need)
		{
			RCLCPP_INFO(node->get_logger(),
			            "Quad and target placed, markers locked for %d frames at sim t=%.3f "
			            "- starting the controllers",
			            lock_run, node->now().seconds());
			rclcpp::shutdown();
			return 0;
		}
		...
		if (waited > timeout_s)
		{
			RCLCPP_ERROR(node->get_logger(),
			             ...
			             "marker_lock=%d/%d. The controllers will NOT be started, because "
