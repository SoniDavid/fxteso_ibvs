...
def _image_qos(kind: str) -> QoSProfile:
    """KEEP_LAST depth-1; BEST_EFFORT for sensor_data, else RELIABLE. Pair with image_features' sensor_qos."""
...
class CameraNode(Node):
    ...
    def __init__(self):
        ...
        # 0.0 -> the sensor mode's max rate, deliberately above the 50 Hz loop so the frame is fresh.
        ...
        # Restarts are rate-limited by time: under CPU contention timeouts do not mean a wedged
        # camera, and each restart adds the very load that caused them.
        ...
        # Cache now: reading .sensor_modes reconfigures the camera, so it must precede configure().
        self._modes = list(self._picam.sensor_modes)

        # picamera2 pins 30 fps unless FrameDurationLimits is set, so it always is.
        if framerate > 0.0:
...
def main(args=None):
    # Before rclpy.init(): a 2.24 MB frame overflows Fast DDS's default 512 KiB segment.
    # The launch files set it too; setdefault keeps a bare `ros2 run` working.
